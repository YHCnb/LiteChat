...
ClientController::ClientController(QObject* parent) : QObject(parent)
{
    ...
    chatMsgMapGroup = QMap<quint32, QList<ChatMessage>*>();
...
//服务端ip
QString ClientController::IP = "127.0.0.1";
...
void ClientController::ConnectToServer() {
    ...
    clientSocket->setServerTcpPort("56666");
...
void ClientController::requestUserInfoByID(quint32 ID) {
	QByteArray content;
	content.clear();
    content.append(QString::number(ID).toUtf8());
